...
public class DAOOrders extends DBContext {
    ...
    public int TotalOrderBySupplier(int supplierID){
        int number  = 0;
        String sql = "SELECT COUNT(OrderDetails.OrderID) FROM dbo.OrderDetails WHERE OrderDetails.ProductID IN (SELECT Products.ProductID FROM dbo.Products WHERE SupplierID = ?)";    
